...
export const fetchSources = createAsyncThunk(
  ...
  async (params: FetchSourcesParams, { rejectWithValue }) => {
    try {
...
export const fetchMoreSources = createAsyncThunk(
  'sources/fetchMoreSources',
  async (params: FetchSourcesParams, { rejectWithValue }) => {
    try {
      logger.debug('Fetching more sources', { params });
      const result = await getSources(params);
      logger.debug('More sources fetched successfully', { count: result.length });
      return result;
    } catch (error) {
      logger.error(logMessages.FETCH_SOURCES_ERROR, error, { params });
      return rejectWithValue(handleThunkError(error, msg.SOURCE_FETCH_ALL_FAILED));
